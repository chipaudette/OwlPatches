...
class FormantShiftingPatch_RL : public Patch {
private:
...
public:
  ...
  void processAudio(AudioBuffer &buffer){
    ...
    //get the latest data
    FloatArray in_buf = buffer.getSamples(1); //get the right channel
    float* out_buf = buffer.getSamples(0); //here's the pointer of where to put the outdata
    
    //store this buffer
    blockBuff_counter++;  //increment
    if (blockBuff_counter >= N_BLOCK_BUFF) blockBuff_counter = 0;  //wrap around
    in_buf.copyTo(blockBuff[blockBuff_counter]); //copy
    ...
    //compute IFFT
    transform.ifft(ca, full_data); //"full_data" is the output
    
    //apply window again
    full_data.multiply(window);
    
    //overlap and add and put into output buffer
    int count=0;
    for (int i=0; i < blockSize; i++) {
         out_buf[i] = gain_scale* (outBlockBuff[0][i]+full_data[count]); //this goes out
         count++;
    }
