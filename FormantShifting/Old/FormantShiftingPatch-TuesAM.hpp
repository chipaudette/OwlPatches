...
public:
  FormantShiftingPatch(){
    //settings.audio_samplingrate = 24000;
    //codec.stop();
    //codec.init(settings);
    //codec.start();
  ...
  void processAudio(AudioBuffer &buffer){
    ...
    for (int i=0; i < blockSize; i++) {
         buf[i] = outBlockBuff[0][i]+full_data[count]; //this goes out
         count++;
    }
    ...
    for (int Iblock = 0; Iblock < (N_BLOCK_BUFF-1); Iblock++) {
        for (int i=0; i < blockSize; i++) {
            outBlockBuff[Iblock][i] = gain_scale*(outBlockBuff[Iblock+1][i]+full_data[count]); //overlap and add
            count++;
        }
    }
